...
# This script contains the necessary code for the analysis. Answer the questions
# using the LinearModels project that we used in the Labs.
...
# Hint for Question 2: The following code provides a partial solution for the
# FertCat.
...
pie(fert_freq,
    ...
    col = c("lightblue", "lightgreen", "lightpink"),  # Choose meaningful colors
    labels = c("Ποσοστό γονιμότητας εφήβων στο (0,50]",
               "Ποσοστό γονιμότητας εφήβων στο (50,100]",
               "Ποσοστό γονιμότητας εφήβων στο (100,Inf)"),  # Add labels for each slice
...
pie(infl_freq,
    ...
    labels = c("Πληθωρισμός ≤ 2%", "Πληθωρισμός > 2%"),  # Labels for the slices
    col = c("lightblue", "lightgreen"),  # Colors for the slices
...
# Hint for Question 4: Use the following code to install and load the package.
# Read the help page to understand how to use the describeBy function.
...
# Hint for Question 1: In the lm function you can use the - sign to exclude a
# variable. You can also use the . sign to include all the variables. See the
# 04_MultipleLinear.R from the LinearModels project.
...
# Hint for Question 8: Use the following line to read the new data set.
